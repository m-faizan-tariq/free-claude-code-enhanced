...
from providers.transports.anthropic_messages.http import maybe_await_aclose
...
class OpenRouterProvider(AnthropicMessagesTransport):
    ...
    def _create_httpx_client(self, step: RotationStep) -> httpx.AsyncClient:
        """Create an httpx client for a rotation step's API key and base URL."""
    ...
    async def _validated_stream_send(
        ...
        if not steps:
            return await super()._validated_stream_send(body, req_tag=req_tag)

        last_error: Exception | None = None
        for i, step in enumerate(steps):
            client = self._create_httpx_client(step)
            try:
                ...
                request = client.build_request(
                    ...
                    headers={
                        ...
                        "Authorization": f"Bearer {step.api_key}",
